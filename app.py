...
<body class="py-4"><nav class="navbar bg-white border-bottom mb-5"><div class="container fw-bold">🚦 Innovexa Catalyst System</div></nav>
...
<div class="col-md-7"><div class="d-flex flex-column h-100 gap-3">
<div class="card p-4 flex-grow-1 d-flex flex-column justify-content-center"><span class="small fw-bold text-muted text-uppercase">Ensemble Forecasted Volume</span><div class="d-flex align-items-baseline gap-2 mt-1"><div class="metric-value" id="volOut">0</div><span class="text-secondary fw-semibold">vehicles / 15-min</span></div></div>
...
<div class="card p-4"><h6 class="fw-bold mb-2 text-dark">🧠 Smart Routing Recommendation</h6><div class="recommendation-box" id="recBox"><p class="mb-0 text-secondary small font-weight-medium" id="routingOut">Waiting for data submission...</p></div></div>
</div></div></div></div>
<script>
...
def predict():
    try:
        ...
        route = "🚨 High saturation. Divert traffic to secondary bypass routes immediately." if pct > 75 else "⚠️ Moderate volume building. Recommend micro-adjusting ramp timers." if pct > 45 else "✅ Traffic flowing smoothly within normal bounds."
